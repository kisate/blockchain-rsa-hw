"""
Взято из моей семетровой работы по алгебре 
"""

import secrets
from math import log, ceil, gcd, sqrt
...
def gen_small_prime():
    while True:
        bits = secrets.choice(range(10, 12))
        n = secrets.randbits(bits)
...
def rsa_gen_keys_small():
    p = gen_small_prime()
    q = gen_small_prime()
    phi = (p-1)*(q-1)
    n = p*q
    e = secrets.randbits(3 + secrets.randbelow(5))
    while gcd(e, phi) != 1:
        e = secrets.randbits(3 + secrets.randbelow(5))
